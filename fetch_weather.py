...
# 警戒レベルの明示的定義（レベル2、3、4、5）
WARNING_CODE_LEVELS = {
    "00": 0,    # 解除
    "02": 3, "03": 3, "04": 3, "05": 3, "06": 3, "07": 3, "08": 3, "09": 3,  # レベル3（警報）
    "10": 2, "18": 2, "19": 2, "29": 2,  # レベル2（警戒レベルに紐づく注意報）
    "32": 5, "33": 5, "35": 5, "36": 5, "37": 5, "38": 5, "39": 5,  # レベル5（特別警報）
    "43": 4, "48": 4, "49": 4   # レベル4（危険警報）
}
...
def get_warning_level(w_code):
    """
    警報コードから警戒レベルを判定する関数
    - コードが null / 空なら None (null) を返す
    - 辞書に定義されているものはそのレベルを返す
    - 定義されていないその他の注意報などは「1」とする
    """
    if not w_code:
        return None
